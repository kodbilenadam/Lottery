...
async function deployContracts() {
  if (contracts.deployed) {
    test.testUSDT = await hre.ethers.getContractAt(
      "TestUSDT",
      contracts.testUSDT
    );
    test.honorTicket = await hre.ethers.getContractAt(
      "HonorTicket",
      contracts.honorTicket
    );
    test.honorLottery = await hre.ethers.getContractAt(
      "HonorLottery",
      contracts.honorLottery
    );
    test.winManager = await hre.ethers.getContractAt(
      "HnrLotoWinManager",
      contracts.winManager
    );
  } else {
    ...
    const honorLottery = await hre.ethers.deployContract("HonorLottery", [
      ticketAddress,
    ]);
    ...
    const winManager = await hre.ethers.deployContract("HnrLotoWinManager", [
      file.honorLottery,
    ]);
...
async function playGame(signer) {
  await test.honorLottery.connect(signer).playGame(0, 5);
}
